...
BEGIN
    ...
    DECLARE @IDDULIEU BIGINT;
    DECLARE @HANHDONG NVARCHAR(20);
    DECLARE @IDNGUOIDUNG BIGINT = SYSTEM_USER;
...
BEGIN
    ...
    BEGIN
        ...
        FROM INSERTED;
    END
    -- Handle UPDATE (including payment confirmation)
    ELSE IF EXISTS (SELECT 1 FROM INSERTED) AND EXISTS (SELECT 1 FROM DELETED)
    BEGIN
        INSERT INTO NHATKY (IDNGUOIDUNG, TENBANG, IDDULIEU, HANHDONG, THOIGIAN)
        SELECT 
            ...
            i.IDTHANHTOAN,
            CASE 
                WHEN i.TRANGTHAI = N'Đã xác nhận' THEN 'XACNHANTHANHTOAN'
                ELSE 'SUA'
            END,
            GETDATE()
        FROM INSERTED i;
